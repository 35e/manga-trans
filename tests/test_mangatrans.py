...
class TestFitting(unittest.TestCase):
    ...
    def test_a_word_too_long_for_the_width_keeps_its_own_line(self):
        font = render.load_font(None, 20)
        self.assertEqual(render.wrap("supercalifragilistic", font, 20), [
            "supercalifragilistic"
        ])

    def test_fit_stays_within_the_box(self):
        box = Box(0, 0, 160, 90)
        font, block, spacing = render.fit(self.draw, "hello there friend", box, None)
    ...
    def test_a_bigger_box_gets_bigger_type(self):
        ...
        self.assertGreater(large[0].size, small[0].size)
...
class TestOverlay(unittest.TestCase):
    def test_the_original_text_goes_and_the_translation_arrives(self):
        image = page_with_text()
        box = Box(45, 35, 155, 85)
        out = np.array(render.overlay(image, None, [(box, "hello there")]))
        was, now = (
            (part[box.y0 : box.y1, box.x0 : box.x1] < 100).sum()
            for part in (np.array(image), out)
        )
        self.assertGreater(now, 0)  # lettered
        self.assertLess(now, was)  # and the heavier Japanese is gone

    def test_a_region_with_no_text_is_only_cleaned(self):
        image = page_with_text()
        box = Box(45, 35, 155, 85)
        out = np.array(render.overlay(image, None, [(box, "  ")]))
        self.assertGreater(out[box.y0 : box.y1, box.x0 : box.x1].min(), 200)

    def test_regions_outside_the_page_are_ignored(self):
        image = page_with_text()
        out = render.overlay(image, None, [(Box(500, 500, 600, 600), "nope")])
...
class TestServer(unittest.TestCase):
    def setUp(self):
        ...
        self.backend = Backend(Pages(root, root / "out"))
