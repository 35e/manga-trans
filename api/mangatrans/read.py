...
The detector says where the text is; this says what it says. manga-ocr is
trained on manga specifically — vertical lines, stylised fonts, furigana — which
is why it manages what general OCR does not. It runs on the CPU.

**The only module here that needs torch**, and the import is deferred to the
first page read, so an API only ever asked to detect or clean never pays for it.
"""
